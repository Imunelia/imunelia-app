(() => {
  ...
  if(!hero)return;
  const load=(id,src)=>new Promise((resolve,reject)=>{const old=document.getElementById(id);if(old){if(old.dataset.loaded==='true')resolve();else old.addEventListener('load',resolve,{once:true});return}const s=document.createElement('script');s.id=id;s.src=src;s.onload=()=>{s.dataset.loaded='true';resolve()};s.onerror=reject;document.head.appendChild(s)});
  if(!document.getElementById('homepage-commercial-style')){const l=document.createElement('link');l.id='homepage-commercial-style';l.rel='stylesheet';l.href='homepage-commercial.css?v=20260803-story4';document.head.appendChild(l)}
  ...
  const copy=en?{
    ...
    items:[
      ['energy','Morning begins in motion.','Energy for a rhythm you set yourself.','produkty-en.html#flow','assets/photos/home-story-energy.jpg?v=20260803-story4'],
      ['performance','When the day asks for more.','For demanding activity, focus and exceptional physical load.','produkty-en.html#ultra','assets/photos/home-story-performance.jpg?v=20260803-story4'],
      ['immunity','Care begins on ordinary days.','An everyday foundation for natural resilience.','produkty-en.html#shield','assets/photos/home-story-immunity.jpg?v=20260803-story4'],
    ...
    items:[
      ['energy','Ráno začíná pohybem.','Energie pro rytmus, který si určujete sami.','produkty.html#flow','assets/photos/home-story-energy.jpg?v=20260803-story4'],
      ['performance','Když je potřeba víc.','Pro náročnou aktivitu, soustředění a mimořádnou fyzickou zátěž.','produkty.html#ultra','assets/photos/home-story-performance.jpg?v=20260803-story4'],
      ['immunity','Péče začíná v běžných dnech.','Každodenní základ pro přirozenou odolnost.','produkty.html#shield','assets/photos/home-story-immunity.jpg?v=20260803-story4'],
  ...
  section.innerHTML=`<div class="home-story__intro"><p class="section-kicker">${copy.kicker}</p><h2>${copy.title}</h2><p>${copy.intro}</p></div><div class="home-story__sequence">${copy.items.map((item,index)=>`<a class="home-story__panel home-story__panel--${item[0]}" href="${item[3]}"><img class="home-story__image"${item[4]?` src="${item[4]}"`:''} alt="" loading="eager" decoding="async"><span class="home-story__shade"></span><span class="home-story__number">0${index+1}</span><span class="home-story__copy"><strong>${item[1]}</strong><span>${item[2]}</span><em>${copy.cta} →</em></span></a>`).join('')}</div><div class="home-story__actions"><a class="button primary" href="${en?'produkty-en.html':'produkty.html'}">${copy.all}</a></div>`;
  hero.insertAdjacentElement('afterend',section);

  section.querySelectorAll('.home-story__image[src]').forEach(img=>img.addEventListener('error',()=>img.closest('.home-story__panel')?.classList.add('image-fallback'),{once:true}));
  ...
  if(window.IMMUNALIA_FAMILY_PHOTO&&window.IMMUNALIA_NIGHT_PHOTO)applyEmbedded();
  else Promise.all([load('family-story-data','assets/js/family-golden-hour-photo.js?v=20260803-story4'),load('recovery-story-data','assets/js/night-premium-evening-photo.js?v=20260803-story4')]).then(applyEmbedded).catch(()=>{});
})();